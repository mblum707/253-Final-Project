...
# probability of being readmitted based on hospital stay length
diabetic_edited_diag %>%
  ggplot(aes(x = number_inpatient)) + 
  geom_bar(aes(fill=factor(readmitted_30)), position = "fill") +
  xlab("Hospital Stay Length (Days)") + ylab("Count") + labs(fill = "Being Readmitted")
...
Also note that the data used to fit this model has been dowmsampled (see method sampling = "down" under trControl). 
...
@MELISSA (we will test both normal and downsampled, but code is included only for normal) - shall we include it too?
@MELISSA This takes forever to run - I have let it work for one hour and it still did not finish. I hope you have a more powerful machine at your end to make this compile!
